...
//	static final int navigationPage = 1;
	...
	public static void main(String[] args)
	{
        ...
        for(Endpoint e : auth.getEndpoints(providerId)) //For the provided endpoint
        {
            ...
            for(XLeaType l : ricOne.xPress.getXLeas().getData()) //Iterate through each xLea
            {
            	for (int i = 1; i <= ricOne.xPress.getLastPage(navigationPageSize, XPress.ServicePath.GETXROSTERSBYXLEA, l.getRefId()); i++) //Get max page size for rosters by lea
        		{
	                for(XRosterType r : ricOne.xPress.getXRostersByXLea(l.getRefId(), i, navigationPageSize).getData()) //Get each roster for each lea refId w/ paging
	                {
